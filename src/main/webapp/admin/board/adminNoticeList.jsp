...
<div class="content_area">
	<table border="0" width="1000" align="center"></table>
		<form id="searchFrm" method="post" action="/adminNoticeList.do?menu=1">
		<input type="hidden" name="pageIndex" id="pageIndex" value="1">
		<select name="searchCondition">
			<option value="noticetitle"
				<c:if test="${PageVO.searchCondition=='noticetitle' }">selected</c:if>
			>제목</option>
			<option value="noticecontent"
				<c:if test="${PageVO.searchCondition=='noticecontent' }">selected</c:if>
			>내용</option>
			<option value="noticerdate"
				<c:if test="${PageVO.searchCondition=='noticerdate' }">selected</c:if>
			>날짜</option>
		</select>
	...
	<c:forEach var="list" items="${NoticeList}" varStatus="status">
		<tr>
			...
			<td align="left" <c:if test="${list.noticeemps eq 'Y'}">
    			class="notice-emphasis"
				</c:if>><a href="adminNoticeDetail.do?noticeunq=${list.noticeunq }"><c:out value="${list.noticetitle}"/></a></td>
			<td <c:if test="${list.noticeemps eq 'Y'}">
    			class="notice-emphasis"
...
<div class="page_number_area">
    ...
    <button onclick="location='/adminNoticeList.do?pageIndex=1&searchCondition=${PageVO.searchCondition}&searchKeyword=${PageVO.searchKeyword}'">first</button>
    <button onclick="location='/adminNoticeList.do?pageIndex=${startIndex}&searchCondition=${PageVO.searchCondition}&searchKeyword=${PageVO.searchKeyword}'">Previous</button>
    	...
    	<!-- 끝나는 인덱스를 10으로 고정  -->
			...
			<c:forEach var="i" begin="${startIndex}" end="${endIndex}">
			    <a href="/adminNoticeList.do?pageIndex=${i }&searchCondition=${PageVO.searchCondition}&searchKeyword=${PageVO.searchKeyword}">${i}</a>
			</c:forEach>
			
	  	 <button onclick="location='/adminNoticeList.do?pageIndex=${endIndex}&searchCondition=${PageVO.searchCondition}&searchKeyword=${PageVO.searchKeyword}'">Next</button>
	  	 <button onclick="location='/adminNoticeList.do?pageIndex=${PageVO.lastPage}&searchCondition=${PageVO.searchCondition}&searchKeyword=${PageVO.searchKeyword}'">last</button>
</div>
<!-- 페이지처리 E -->
